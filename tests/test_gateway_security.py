...
def test_create_mfa_challenge_stores_user_id_with_a_short_fixed_ttl(monkeypatch):
    fake = _FakeRedis()
    monkeypatch.setattr(security, "_redis", lambda: fake)

    token = security.create_mfa_challenge(7)

    assert token
    key, mapping = fake.hset_calls[0]
    assert key == f"mfa_challenge:{token}"
    assert mapping == {"user_id": "7"}
    assert fake.expire_calls == [(key, security.settings.mfa_challenge_timeout_seconds)]


def test_get_mfa_challenge_returns_the_user_id_as_an_int(monkeypatch):
    key = "mfa_challenge:tok"
    fake = _FakeRedis(store={key: {"user_id": "7"}})
    monkeypatch.setattr(security, "_redis", lambda: fake)

    assert security.get_mfa_challenge("tok") == 7
...
def test_get_mfa_challenge_never_refreshes_ttl(monkeypatch):
    # Unlike a session, a challenge is single-purpose and short-lived by
    # design — reading it must never extend its life.
    key = "mfa_challenge:tok"
    fake = _FakeRedis(store={key: {"user_id": "7"}})
    monkeypatch.setattr(security, "_redis", lambda: fake)
...
def test_destroy_mfa_challenge_deletes_it(monkeypatch):
    fake = _FakeRedis()
    monkeypatch.setattr(security, "_redis", lambda: fake)

    security.destroy_mfa_challenge("tok")

    assert fake.deleted == ["mfa_challenge:tok"]
